...
export class FhirDefinition {
        ...
        await Promise.all(
            dependencies.map(async (dependency) => {
                ...
                if (/^\d+\.\d+\.x$/.test(dependency.version)) {
                    resolvedVersion = await this.resolvePatchVersion(dependency.packageId, dependency.version);
                } else {
                ...
                try {
                    ...
                    if (!fileSystem.existsSync(path.join(this.cachePath, packageKey))) {
                        ...
                        if (download) {
                            const packageInput = path.join(
                                os.homedir(),
                                ".fhir",
                                `${dependency.packageId}-${resolvedVersion}.tgz`,
                            );
                    ...
                    this.logger.appendLine(
                        `${new Date().toLocaleString("fr-FR")} : Could not load definition information for package ${packageKey}`,
                    );
                }
                return;
            }),
    ...
    public async resolvePatchVersion(packageId: string, version: string): Promise<string> {
        ...
        const potentialVersions = cacheContents
            .filter(([fileName, type]) => type === FileType.Directory && fileName.startsWith(`${packageId}#`))
        ...
        snapshotElements.forEach((element) => {
            ...
            if (pathParts.length > 0) {
                ...
                if (parent !== null && !parent.some((existing) => existing.path === pathParts[0])) {
                    if (
                        (typeof element.max === "string" && element.max !== "0") ||
                        (typeof element.max === "number" && element.max > 0)
                    ) {
                        parent.push({
    ...
    private async getPackagesRegistry(): Promise<string[]> {
        ...
        if (fs.existsSync(path.join(os.homedir(), ".fhir", "fhir-settings.json"))) {
            const settingsContents = await workspace.fs.readFile(
                Uri.file(path.join(os.homedir(), ".fhir", "fhir-settings.json")),
            );
            const decoder = new TextDecoder();
