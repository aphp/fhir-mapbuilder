...
export function downloadFHIRPackage(
    ...
    logger.appendLine(
        `${new Date().toLocaleString("fr-FR")} : Start downloadFHIRPackage for ${packageName}#${version}`,
    );
    let download = false;
    ...
    registries.forEach((url) => {
        try {
            ...
            logger.appendLine(
                `${new Date().toLocaleString("fr-FR")} : Error downloading package ${packageName}#${version} : ${er}`,
            );
...
export function collectFilesWithExtension(filepath: string, files: string[], extension: string): void {
...
export async function getAllParamsFromUrl(
    logger: OutputChannel,
    url: string,
    definitionProvider: FhirDefinition,
