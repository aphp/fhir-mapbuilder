...
export class FmlCompletionProvider implements CompletionItemProvider {
    ...
    constructor(
        private definitionProvider: FhirDefinition,
    ...
    public async scanAll(): Promise<void> {
        ...
        workspace.workspaceFolders!.forEach((folder) => {
            ...
            this.logger.appendLine(
                `${new Date().toLocaleString("fr-FR")} : Start collectFilesFromPath for ${folderPath}`,
            );
            collectFilesWithExtension(folderPath, fmlFiles, ".fml");
            this.logger.appendLine(
                `${new Date().toLocaleString("fr-FR")} : End collectFilesFromPath for ${folderPath}`,
            );
        });
        ...
        fmlFiles.forEach((fmlFile) => {
            ...
            if (groupLine.length > 0) {
                ...
                returnLines.forEach((line) => {
                    ...
                    const objToUpdate = this.userDefinitions
                        .get(fmlFile)
                        ?.find((obj) => obj.as === element && obj.alias === alias);
                    if (objToUpdate) {
