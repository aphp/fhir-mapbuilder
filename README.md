...
| **`audit.yml`** | Weekly cron, manual | OSV-Scanner run over `pom.xml` + `package-lock.json`; opens a deduplicated `dependencies` issue on a real finding and uploads the SARIF to the Security tab. A finding does not fail the job; only a scan that cannot run does. | [0002](docs/adr/0002-chaine-ci-cd.md) |
